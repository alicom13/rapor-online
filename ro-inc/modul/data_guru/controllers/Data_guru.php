...
class Data_guru extends CI_Controller {
    ...
    public function datatable() {
        ...
        foreach ($q_datanya as $d) {
            ...
            $data_ok[0] = $no++;
            $data_ok[1] = $d['nip'];
            $data_ok[2] = $d['jml_aktif'] > 0 ? $d['nama']." / <b><i>".$d['username']."</i></b> / <b>NIP guru</b>" : $d['nama'];
    ...
    public function aktifkan($id) {
        ...
        $detil_data = $this->db->query("SELECT nip FROM m_guru WHERE id = '".$id."'")->row_array();
        ...
        if (empty($detil_data)) {
          ...
          $username = strtoupper(str_replace(array(".",","," "), array("","",""), "RO".$detil_data['nip']));
            ...
            $this->db->query("INSERT INTO m_admin (username,password,level,konid,aktif) VALUES ('".$username."', '".$password."', 'guru', '$id', 'Y')");

            $d['status'] = "ok";
            //$d['data'] = "Username : ".$username_fix." berhasil diaktifkan..! Password default guru123";
            $d['data'] = "Username : ".$username." berhasil diaktifkan..! Password Awal : ".$username."";
        }
    ...
    public function nonaktifkan($id) {
        ...
        if (empty($detil_data)) {
            ...
            //$username = strtolower(str_replace(array(".",","," "), array("","",""), $detil_data['nama']));
            $username = strtoupper(str_replace(array(".",","," "), array("","",""), $detil_data['nip']));
            //$password = sha1(sha1('guru123'));
          	$password = sha1(sha1($username));
    ...
    public function index() {
    	$this->d['p'] = "list";
        $this->d['p_isbk'] = array("2"=>"Tidak","1"=>"Ya");
